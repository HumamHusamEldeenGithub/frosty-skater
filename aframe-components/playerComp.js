...
var centerWeight = 1, leftWeight = 0, rightWeight = 0;
...
export const playerComp = {
  ...
  async movementController(timeDelta) {
    ...
    angle < 0? Math.max(-10, angle) : Math.min(10, angle);

    var position = this.model.object3D.position;
    if (Math.abs(position.x - angle * timeDelta) < 10)
      angle > 0 ? this.model.object3D.position.x -= 12 * percent * timeDelta
        : this.model.object3D.position.x += 12 * percent * timeDelta;
